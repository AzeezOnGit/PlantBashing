...
while [[ "$keep_playing" == "true" ]]; do
    ...
    if [[ "$name_choice" == "y" || "$name_choice" == "yes" ]]; then
    ...
    while [ $plant_age -lt 21 ] && [[ "$keep_playing" == "true" ]]; do
        grow_plant
        read -p "Press y to continue to the next day, or n to quit: " next_choice
        if [[ "$next_choice" == "n" || "$next_choice" == "no" ]]; then
            echo "Okay, ending the game early. Thanks for playing, $user_name!"
            break
        fi
    done
